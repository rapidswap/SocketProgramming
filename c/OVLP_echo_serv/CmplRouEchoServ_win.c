...
int main(int argc, char* argv[])
{
	...
	ioctlsocket(hLisnSock, FIONBIO, &mode);//non-blocking mode
	...
	while (1)
	{
		...
		lpOvLp = (LPWSAOVERLAPPED)malloc(sizeof(WSAOVERLAPPED)); //데이터가 왔다갔다 하기때문에 동적할당
...
void CALLBACK WriteCompRoutine(DWORD dwError, DWORD szRecvBytes, LPWSAOVERLAPPED lpOverlapped, DWORD flags)
{
	...
	{
		bufInfo->len = szRecvBytes;
		WSASend(hSock, bufInfo, 1, &sentBytes, 0, lpOverlapped, ReadCompRoutine);
	}
