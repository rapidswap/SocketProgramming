...
//WSAOVERLAPPED 구조체를 확장해서 필요한 정보를 더넣는다.
...
public:
	...
	bool InitSocket()
	{
		...
		//연결지향형 TPC, Overlapped I/O 소켓 생성
	...
	//서버용 함수
	//서버의 주소정보를 소켓과 연결시키고 접속 요청을 받기위해 등록한 함수
	bool BindandListen(int nBindPort)
	{
		SOCKADDR_IN stServerAddr;
		stServerAddr.sin_family = AF_INET;
		stServerAddr.sin_port = htons(nBindPort); //서버포트를 설정한다.
		//어떤 주소에서 들어오는 접속이라도 받아들이겠다.
		//보통 서버라면 이렇게 설정, 만약 한 아이피에서만 접속을 받고 싶다면
		//그 주소를 inet_addr 함수를 이용해 넣으면됨.
		...
		//접속 요청을 받아들이기 위해 cIOCompletionPort 소켓을 등록하고 접속대기큐를 5개로 설정한다.
	...
	bool StartServer(const UINT32 maxClientCount)
	{
		...
		if (NULL == mIOCPHandle)
		{
			printf("[에러] CreateClient()함수 실패 : %d\n", WSAGetLastError());
			return false;
		}
...
private:
	...
	bool SendMsg(stClientInfo* pClientInfo, char* pMsg, int nLen)
	{
		...
		//Overlapped I/O을 위해 각 정보를 셋팅해준다.
	...
	//Overlapped I/O작업에 대한 완료 통보를 받아 그에 해당하는 처리르 하는 함수
	void WokerThread()
	{
	...
	void AccepterThread()
	{
		...
		while (mIsAccepterRun)
		{
			...
			//I/OCompletion Port 객체와 소켓을 연결시킨다.
	...
	void CloseSocket(stClientInfo* pClientInfo, bool blsForce = false)
	{
		...
		//blsForce가  true이면 SO_LINGER, timeout =0으로 설정하여 강제 종료 시킨다. 주의 : 데이터 손실이 있을 수 있음.
		if (true == blsForce)
		{
			stLinger.l_onoff = 1;
		}
